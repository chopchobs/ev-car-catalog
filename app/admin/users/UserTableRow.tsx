          ...
          <div
            ...
            ${
              isRoleAdmin
                ...
                : "bg-linear-to-br from-emerald-400 to-teal-500 shadow-teal-500/20"
            }`}
          >
            {getInitials(user.name)}
          </div>
          <div>
            <p className="font-bold text-gray-900 leading-tight truncate max-w-[150px]">
              {user.name || "Member"}
            </p>
            {isSelf && (
              <span className="text-[10px] bg-gray-100 text-gray-500 px-2 rounded-full font-bold ml-1 inline-block mt-0.5">
                ตัวคุณเอง
              </span>
            )}
          </div>
      ...
      <td className="px-6 py-4">
        {isSelf ? (
          ...
          <div className="relative inline-block w-full max-w-[140px]">
            ...
            {/* ไอคอนลูกศร */}
            <div
              className={`pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 ${isRoleAdmin ? "text-indigo-400" : "text-emerald-400"}`}
            >
              <svg
