  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">จัดการข้อมูลรถยนต์</h2>
          <p className="text-gray-500 text-sm mt-1">รายการรถยนต์ทั้งหมดในระบบ (พบ {cars.length} คัน)</p>
        </div>
        <Link 
          href="/admin/cars/add" 
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-lg font-medium text-gray-900">ยังไม่มีข้อมูลรถยนต์</p>
                      <p className="text-sm mt-1">คลิกปุ่ม "เพิ่มรถคันใหม่" ด้านบนเพื่อเริ่มต้น</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={car.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="w-20 h-14 rounded-md overflow-hidden bg-gray-100 border border-gray-200 mx-auto flex items-center justify-center">
                        {car.coverImage ? (
                          <img src={car.coverImage} alt={car.modelName} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <td className="p-4 text-gray-700">
                      <div>{car.year}</div>
                      <div className="text-sm text-gray-500">{car.mileage.toLocaleString()} กม.</div>
                    </td>
                    ...
                    <td className="p-4 text-right space-x-3">
                      <Link href={`/admin/cars/edit/${car.id}`} className="text-blue-600 hover:text-blue-900 font-medium text-sm">
                        แก้ไข
                      </Link>
