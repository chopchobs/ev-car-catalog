...
export default async function ComparePage({
    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex flex-col">
        ...
        <main className="flex-1 container mx-auto px-6 py-24 text-center">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">ไม่มีรถที่เลือกเปรียบเทียบ</h1>
  ...
  const sortedCars = carIdsArray.map(id => cars.find(c => c.id === id)).filter(Boolean) as typeof cars;
      ...
      <div className="pt-32 pb-16 bg-white border-b border-neutral-200">
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <p className="text-lg text-gray-500 max-w-2xl font-medium">
            ข้อมูลเชิงลึกเปรียบเทียบสเปครถยนต์ไฟฟ้าที่คุณสนใจ เพื่อการตัดสินใจที่ดีที่สุด
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <thead>
                <tr className="border-b-2 border-gray-100">
                  <th className="w-48 p-6 bg-gray-50 align-bottom">
                    <p className="font-bold text-gray-500 text-sm uppercase tracking-wider mb-2">รุ่นที่เลือก</p>
                    <p className="text-3xl font-black text-gray-900">{sortedCars.length} คัน</p>
                  </th>
                  ...
                  {Array.from({ length: 3 - sortedCars.length }).map((_, i) => (
                    <th key={`empty-th-${i}`} className="p-6 align-top w-1/3 opacity-50">
                      <div className="aspect-[4/3] w-full border-2 border-dashed border-gray-200 rounded-2xl flex items-center justify-center bg-gray-50">
                        <span className="text-gray-400 font-medium">เพิ่มรถเพื่อเปรียบเทียบ</span>
                      </div>
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                <tr className="hover:bg-blue-50/30 transition-colors">
                  <td className="p-6 font-semibold text-gray-900 bg-gray-50/50">ปีที่ผลิต / Year</td>
                  {sortedCars.map(car => (
                    <td key={`year-${car.id}`} className="p-6 font-medium text-gray-600">
                      {car.year}
                    </td>
                  ))}
                  {Array.from({ length: 3 - sortedCars.length }).map((_, i) => (
                    <td key={`year-empty-${i}`} className="p-6 bg-gray-50/20"></td>
                  ))}
                </tr>

                {/* แถว: เลขไมล์ */}
                <tr className="hover:bg-blue-50/30 transition-colors">
                  <td className="p-6 font-semibold text-gray-900 bg-gray-50/50">เลขไมล์ / Mileage</td>
                  {sortedCars.map(car => (
                    <td key={`mileage-${car.id}`} className="p-6 font-medium text-gray-600">
                      {new Intl.NumberFormat("th-TH").format(car.mileage)} กม.
                    </td>
                  ))}
                  {Array.from({ length: 3 - sortedCars.length }).map((_, i) => (
                    <td key={`mile-empty-${i}`} className="p-6 bg-gray-50/20"></td>
                  ))}
                </tr>

                {/* แถว: สีรถ */}
                <tr className="hover:bg-blue-50/30 transition-colors">
                  <td className="p-6 font-semibold text-gray-900 bg-gray-50/50">สีภายนอก / Color</td>
                  {sortedCars.map(car => (
                    <td key={`color-${car.id}`} className="p-6 font-medium text-gray-600">
                      {car.color || "-"}
                    </td>
                  ))}
                  {Array.from({ length: 3 - sortedCars.length }).map((_, i) => (
                    <td key={`col-empty-${i}`} className="p-6 bg-gray-50/20"></td>
                  ))}
                </tr>
                ...
                <tr className="hover:bg-blue-50/30 transition-colors">
                  <td className="p-6 font-semibold text-gray-900 bg-gray-50/50 align-top">
                    ฟีเจอร์เด่น<br/><span className="text-gray-400 text-sm font-normal">Features</span>
                  </td>
                  {sortedCars.map(car => (
                    <td key={`feat-${car.id}`} className="p-6 align-top">
                      <ul className="space-y-2">
                        ...
                        {car.features.length > 5 && (
                          <li className="text-sm text-gray-400 italic">...และอื่นๆ</li>
                        )}
                      </ul>
                    </td>
                  ))}
                  {Array.from({ length: 3 - sortedCars.length }).map((_, i) => (
                    <td key={`feat-empty-${i}`} className="p-6 bg-gray-50/20"></td>
                  ))}
                </tr>
              </tbody>
