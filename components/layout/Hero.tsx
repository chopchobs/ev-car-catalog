...
export default function Hero({ showSearch = true }: { showSearch?: boolean }) {
  ...
  return (
    <div className={`relative w-full min-h-[40vh] bg-[#f4f8ff] flex flex-col items-center justify-center text-center px-4 overflow-hidden py-16 md:py-24 ${!showSearch && 'pb-24 md:pb-32'}`}>
      
      {/* ---------------- Background Glowing Orbs ---------------- */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-200/50 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-cyan-200/50 rounded-full blur-[100px] pointer-events-none"></div>

      {/* ---------------- Text Content ---------------- */}
      <h2 className="text-blue-600 font-semibold text-xl md:text-2xl mb-4 tracking-widest uppercase">
        ขับเคลื่อนสู่อนาคต
      </h2>
      
      {/* ล็อกความสูงไว้ เพื่อไม่ให้หน้าเว็บขยับขึ้นลงตอนตัวอักษรเปลี่ยน */}
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-black mb-6 flex items-center justify-center min-h-[60px] md:min-h-[80px]">
        {displayedText}
        <span className="animate-pulse ml-2 font-light text-black">|</span>
      </h1>
      
      <p className={`max-w-2xl text-lg md:text-xl text-slate-600 leading-relaxed z-10 ${showSearch ? 'mb-12' : 'mb-4'}`}>
      ...
      {showSearch && (
        <form onSubmit={handleSearch} className="w-full max-w-3xl bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-500 p-[2px] rounded-full shadow-lg shadow-blue-500/10 hover:shadow-xl hover:shadow-blue-500/20 transition-all z-10">
          <div className="w-full bg-white rounded-full flex items-center px-4 py-3 md:py-4">
            
            <svg className="w-6 h-6 text-blue-500 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              className="flex-1 outline-none text-slate-800 bg-transparent text-base md:text-lg placeholder-slate-400 w-full"
            />

            <button type="submit" className="flex items-center text-slate-400 text-sm md:text-base border-l-2 border-slate-100 pl-3 md:pl-4 ml-2 flex-shrink-0 group cursor-pointer bg-transparent border-t-0 border-r-0 border-b-0 outline-none">
              <span className="hidden sm:block mr-2 font-medium group-hover:text-blue-600 transition-colors">EV Search</span>
