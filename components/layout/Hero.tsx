...
export default function Hero({ showSearch = true }: { showSearch?: boolean }) {
  ...
  return (
    <div
      className={`relative w-full min-h-[40vh] bg-[#f4f8ff] dark:bg-slate-900 flex flex-col items-center justify-center text-center px-4 overflow-hidden py-16 md:py-24 ${!showSearch && "pb-24 md:pb-32"}`}
    >
      ...
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-black dark:text-white mb-6 flex items-center justify-center min-h-[60px] md:min-h-[80px]">
        {displayedText}
        <span className="animate-pulse ml-2 font-light text-black dark:text-white">
          |
        </span>
      </h1>

      <p
        className={`max-w-2xl text-lg md:text-xl text-slate-600 dark:text-slate-300 leading-relaxed z-10 ${showSearch ? "mb-12" : "mb-4"}`}
      >
        ศูนย์รวม EV Car มือสอง คัดสรรคุณภาพ
        เพื่อประสบการณ์ขับขี่ที่ดีที่สุดของคุณ
      </p>
      ...
      {showSearch && (
        <form
          ...
          className="w-full max-w-3xl bg-linear-to-r from-blue-400 via-cyan-400 to-blue-500 p-[2px] rounded-full shadow-lg shadow-blue-500/10 hover:shadow-xl hover:shadow-blue-500/20 transition-all z-10"
        >
          <div className="w-full bg-white dark:bg-slate-900 rounded-full flex items-center px-4 py-3 md:py-4">
            <svg
              className="w-6 h-6 text-blue-500 dark:text-cyan-400 mr-3 shrink-0"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <button
              type="submit"
              className="flex items-center text-slate-400 dark:text-slate-400 text-sm md:text-base border-l-2 border-slate-100 dark:border-slate-700 pl-3 md:pl-4 ml-2 shrink-0 group cursor-pointer bg-transparent border-t-0 border-r-0 border-b-0 outline-none"
            >
              <span className="hidden sm:block mr-2 font-medium group-hover:text-blue-600 dark:group-hover:text-cyan-400 transition-colors">
                EV Search
              </span>
