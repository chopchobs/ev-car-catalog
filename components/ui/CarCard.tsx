...
export default function CarCard({ car }: { car: Car }) {
  ...
  const getStatusBadge = (status: Car["status"]) => {
    switch (status) {
      case "AVAILABLE":
        return (
          <span className="bg-emerald-500/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-semibold shadow-sm tracking-wide">
            พร้อมขาย
          </span>
        );
      case "BOOKED":
        return (
          <span className="bg-orange-500/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-semibold shadow-sm tracking-wide">
            จองแล้ว
          </span>
        );
      case "SOLD":
        return (
          <span className="bg-gray-500/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-semibold shadow-sm tracking-wide">
            ขายแล้ว
          </span>
        );
    }
  ...
  return (
    <div className="group bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-xl hover:border-cyan-200 dark:hover:border-cyan-800 transition-all duration-300 hover:-translate-y-1 flex flex-col">
      ...
      <div className="relative aspect-4/3 w-full bg-gray-50 dark:bg-slate-800/50 overflow-hidden group/gallery">
        ...
        <FavoriteButton
          carId={String(car.id)}
          initialIsFavorite={car.isFavorite}
        />
        {hasImages ? (
          <>
            <img
      ...
      <div className="p-5 flex-1 flex flex-col">
        {/* ยี่ห้อ และ รุ่น */}
        <div className="mb-3">
          <p className="text-xs font-bold text-cyan-600 dark:text-cyan-400 uppercase tracking-wider mb-1">
            {car.brand}
          </p>
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 dark:border-slate-800 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-400 dark:text-slate-500 mb-0.5 font-medium">
                ราคา
              </p>
