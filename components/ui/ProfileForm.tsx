...
export default function ProfileForm({ initialName, email }: ClientProfileFormProps) {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 p-8 sm:p-10 mb-8 overflow-hidden relative">
      {/* Decorative Blob */}
      ...
      <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
        ...
        <div>
          <h3 className="text-lg font-black text-gray-900 mb-4 border-b border-gray-100 pb-3 flex items-center gap-2">
            <svg className="w-5 h-5 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            ข้อมูลทั่วไป
          </h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">อีเมลเข้าสู่ระบบ (ระบบล็อกไว้)</label>
              <input 
                type="email" 
                value={email} 
                disabled 
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">ชื่อ - นามสกุลที่แสดงผล</label>
              <input 
                name="name" 
                type="text" 
        ...
        <div>
          <h3 className="text-lg font-black text-gray-900 mb-4 border-b border-gray-100 pb-3 flex items-center gap-2 mt-10">
            <svg className="w-5 h-5 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
          ...
          <p className="text-sm text-gray-500 font-medium mb-6 bg-blue-50/50 p-3 rounded-xl border border-blue-100/50">
            💡 หากคุณต้องการแก้ไขแค่ชื่อนามสกุล ไม่จำเป็นต้องกรอกฟอร์มด้านล่างนี้ ปล่อยให้ว่างไว้ได้เลยครับ
          </p>

          <div className="space-y-5">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">รหัสผ่านปัจจุบัน <span className="text-gray-400 font-normal">(เพื่อยืนยันตัวตน)</span></label>
              <input 
        ...
        <div className="pt-6">
          <button 
            type="submit" 
            disabled={isLoading}
            className="px-8 py-3.5 bg-gradient-to-r from-gray-900 to-gray-800 hover:from-cyan-600 hover:to-blue-600 text-white font-bold rounded-xl shadow-lg transition-all hover:-translate-y-0.5 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? (
               <>
